...
Explore the collection at [skills.lab.sa](https://skills.lab.sa/). The SvelteKit directory prerenders the complete Skill and Recipe catalog, then adds fast local search and category filtering without another content request. The dedicated `/recipes/` index exposes every workflow and phase through the same filter system while retaining `/recipes.html` as a generated compatibility page. Every Recipe route renders its complete source-backed phases, steps, prompts, handoffs, and skill references while `/recipe.html` remains a generated compatibility page for the Functioning Prototype. Every Skill route includes its installation command, complete safe instructions, package files, source attribution, and deterministic related content in the initial HTML. The canonical build also derives `llms.txt`, schema-v2 catalogs, exact Markdown mirrors, and a license mirror from that same content snapshot; noncanonical and GitHub Pages builds deliberately remain HTML-only. The collection begins with a functioning-prototype delivery path that combines LABs and external skills through focused chat handoffs. Every push to `master` rebuilds the site on Netlify.
